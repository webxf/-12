...
export default {
    ...
    methods: {
        ...
        async getTableDetail () {
            const res = await this.$service.environmentInspection.getTableDetail({ 'tableId': this.id });
            let list =[]; 
        ...
        // 保存巡检表
        async save(arr){
            const res = await this.$service.environmentInspection.postSaveTable({tableId:this.tableId, menuVos:arr});
            uni.showToast({
