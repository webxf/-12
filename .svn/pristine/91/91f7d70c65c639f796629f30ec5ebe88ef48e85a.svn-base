...
export default {
    ...
    methods: {
        ...
        async getDailyList () {
            ...
            const res = await this.$service.environmentInspection.getDailyList({ modelType: 2, ...this.queryParams, xjzt: this.tabList[this.currentIndex].value });
        ...
        handleCheck (info) {
            uni.navigateTo({
                url: '/packPages/environment-inspection/check-list?id=' + info.tableId+'&type='+this.tabList[this.currentIndex].value
            })
        },
        reset () {
            this.queryParams.pageNo = 1
