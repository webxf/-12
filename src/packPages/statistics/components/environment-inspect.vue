   ...
   <u--form labelPosition="left" labelWidth="40" :model="queryParams" style="display:flex;align-items:center">
          <u-form-item label="时间" @click="timePicker = true" style="margin-left:10px">
             ...
             <u-form-item label="状态" style="margin-left:10px">
              <input placeholder="请输入状态" style="border:1px solid #ccc;width:240rpx;height:26px;border-radius:5px;font-size:13px;background:#fff;padding-left:5px"/>
              </u-form-item>
      </u--form>
         <card :titleStyle="titleStyle" style="margin-left:10px;margin-top:10px;width:94%" v-for="item in inspectDate" :key="item.alertId">
			...
			<view class="item-text edit">
			    <view>巡查操作：</view>
                <view style="display:flex" v-for="(i,index) in item.btns" :key="index">
                    <button style="background:rgb(22,155,213)" @click="goDetail(item.alertId)" v-if="i == 'xq'">详情</button>
                    <!-- <button style="background:rgb(22,155,213)" @click="goDetail" v-if="i == 'xp'">下派</button> -->
                    <button style="background:rgb(22,155,213)" @click="edit(item.alertId)" v-if="i == 'cz'">处置</button>
