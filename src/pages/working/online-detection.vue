<template>
  <view class="container">
    ...
    <view>
      <card
        :title="queryParams.pkmc"
        :value="queryParams.pklx"
    ...
    <view class="tag-box">
      <u-tag
        text="分均"
        @click="switchTab(1)"
        :type="queryParams.sjlx == 1 ? 'primary' : 'info'"
        plain
        shape="circle"
      ></u-tag>
      <u-tag
        text="时均"
        @click="switchTab(2)"
        :type="queryParams.sjlx == 2 ? 'primary' : 'info'"
        plain
        shape="circle"
      ></u-tag>
      <u-tag
        text="日均"
        @click="switchTab(3)"
        :type="queryParams.sjlx == 3 ? 'primary' : 'info'"
        plain
        shape="circle"
      ></u-tag>
    </view>
  ...
  methods: {
    ...
    async getMonitorData() {
      ...
      let list = data.map((item) => {
        return titles.map((iitem) => ({
          label: iitem.name,
          value: item[iitem.prop],
        }));
      });
